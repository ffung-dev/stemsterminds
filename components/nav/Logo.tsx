...
export function Logo({ organizationName, logo }: { organizationName: string; logo?: ImageData }) {
  return (
    ...
    >
      {logo?.url ? (
        <span className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-ink bg-surface">
          <Image src={logo.url} alt={logo.alt || organizationName} fill className="object-cover" />
        </span>
      ) : (
